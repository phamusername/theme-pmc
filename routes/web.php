...
use KKPhim\ThemePmc\Controllers\ThemePmcController;
...
Route::group([
    ...
    Route::post(sprintf('/%s/{movie}/{episode}-{id}/report', config('kkphim.routes.movie', 'phim')), [ThemePmcController::class, 'reportEpisode'])
        ->where(['movie' => '.+', 'episode' => '.+', 'id' => '[0-9]+'])->name('episodes.report');
    Route::post(sprintf('/%s/{movie}/rate', config('kkphim.routes.movie', 'phim')), [ThemePmcController::class, 'rateMovie'])->name('movie.rating');
